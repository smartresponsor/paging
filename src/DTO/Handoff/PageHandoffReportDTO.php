 ...
 */
final readonly class PageHandoffReportDTO
{
    /** @param list<PageHandoffItemDTO> $items */
    ...
    {
        return count(array_filter($this->items, static fn (PageHandoffItemDTO $item): bool => 'ready' === $item->status));
    }
    ...
    public function toArray(): array
    {
        return [
            ...
            'status' => $this->status(),
            'items' => array_map(
                static fn (PageHandoffItemDTO $item): array => $item->toArray(),
                $this->items,
            ),
            'next_owner_layers' => [
